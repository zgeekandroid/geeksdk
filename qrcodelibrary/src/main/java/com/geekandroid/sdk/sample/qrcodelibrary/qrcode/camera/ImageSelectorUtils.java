package com.geekandroid.sdk.sample.qrcodelibrary.qrcode.camera;
...
import com.commonslibrary.commons.utils.DateUtils;
...
public class ImageSelectorUtils {
    ...
    private ImageSelectorUtils(Activity ctx) {
        context = ctx;
    }


    public static ImageSelectorUtils createSelector(Activity ctx) {
        if (context == ctx) {
            return instance;
        }
        instance = new ImageSelectorUtils(ctx);
        return instance;
    }
    ...
    public void selectImage(final SelectorCallBack callBack) {
        ...
        new AlertDialog.Builder(context).setTitle("选择头像")
    ...
    public File saveBitmap(final Bitmap mBitmap) {
        ...
        try {
            ...
            if (createSuccess) {
                LogUtils.i("创建文件" + (createSuccess ? "成功" : "失败"));
            }
